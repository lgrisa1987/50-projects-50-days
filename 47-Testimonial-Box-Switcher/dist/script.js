...
var select = function select(el) {
  ...
  return answerEls.reduce(function (acc, answer) {
    if (answer.checked) return answer.id;else return acc;
  }, 0);
};
...
submitBtn.addEventListener("click", function () {
  ...
  if (answer) answer === quizData[currentQuiz].correct && score++;else {
    alert("Choose an option to continue");
    return;
  }
  currentQuiz++;
  if (currentQuiz < quizData.length) loadQuiz();else quiz.innerHTML = "<h2>You answered ".concat(score, "/").concat(quizData.length, " questions correctly</h2><button onclick=\"location.reload();\">Reload</button>");
});